class StarTraderController < ApplicationController
  ...
  def filtered_market_scope
    ...
    when "buy"
      scope = scope.player_can_buy
    when "sell"
      scope = scope.player_can_sell
    when "both"
      scope = scope.player_can_buy.player_can_sell
    end
  ...
  def sort_column
    {
      ...
      "buy_price" => ProductionFacility::PLAYER_BUY_PRICE_SQL,
      "sell_price" => ProductionFacility::PLAYER_SELL_PRICE_SQL,
      "inventory" => "production_facilities.inventory",
      "demand" => "production_facilities.scu_buy",
      "profit_margin" => "(#{ProductionFacility::PLAYER_SELL_PRICE_SQL} - #{ProductionFacility::PLAYER_BUY_PRICE_SQL})",
  ...
  def market_overview
    {
      best_buy: ProductionFacility.player_can_buy.where("#{ProductionFacility::PLAYER_BUY_PRICE_SQL} > 0").order(Arel.sql("#{ProductionFacility::PLAYER_BUY_PRICE_SQL} ASC")).includes(:commodity, :location).first,
      best_sell: ProductionFacility.player_can_sell.where("#{ProductionFacility::PLAYER_SELL_PRICE_SQL} > 0").order(Arel.sql("#{ProductionFacility::PLAYER_SELL_PRICE_SQL} DESC")).includes(:commodity, :location).first,
