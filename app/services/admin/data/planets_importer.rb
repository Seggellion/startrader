  ...
  module Data
    class PlanetsImporter
      ...
      def self.import_raw_json!(json_string)
        ...
        Rails.logger.info "Successfully imported #{imported_count} planets via raw JSON."
        imported_count
      rescue JSON::ParserError => e
        Rails.logger.error "Failed to parse pasted JSON (PlanetsImporter): #{e.message}"
        0
      rescue => e
      ...
      def self.import_location(location_data)
        ...
        location.assign_attributes(
          ...
          parent_name: location_data['star_system_name'],
